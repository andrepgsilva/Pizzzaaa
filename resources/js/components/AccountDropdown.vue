<template>
  <div class="relative">
    <button @click="isOpen = !isOpen" class="relative z-10 block h-8 w-8 rounded-full overflow-hidden border-2 border-gray-600 focus:outline-none ">
      ...
      <img class="h-full w-full object-cover" :src="profilePhotoPath" alt="Your avatar">
    </button>
    ...
    <div v-if="isOpen" class="absolute right-0 mt-2 py-2 w-48 bg-white rounded-lg shadow-xl">
      <a href="#" @click.prevent="SignOutCallback" class="block px-4 py-2 text-gray-800 hover:bg-gray-100">Sign out</a>
    </div>
  </div>
</template>
